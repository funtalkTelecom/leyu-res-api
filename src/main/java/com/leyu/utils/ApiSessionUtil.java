...
import com.leyu.config.utils.RedisUtil;
...
public class ApiSessionUtil {
    ...
    public static final String JESSION_ID_NAME = "token";
...
//        return StringUtils.defaultIfEmpty(request.getParameter(JESSION_ID_NAME),String.valueOf(request.getAttribute(JESSION_ID_NAME)));

        return StringUtils.defaultIfEmpty(request.getHeader(JESSION_ID_NAME),String.valueOf(request.getHeader(JESSION_ID_NAME)));
    ...
    public void saveSessionKey(String apiKey,String SessionKey) {
        if(apiKey == null) return ;
        String key1 = getApiKey(apiKey);
        redisUtils.set(key1,SessionKey,expire_time);
    }

    public String getSessionKey(String apiKey){
        if(apiKey == null) return null;
        String key = getApiKey(apiKey);
